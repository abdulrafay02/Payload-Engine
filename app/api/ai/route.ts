...
const getVehicleContext = (vehicle?: VehicleConfig) => {
  if (!vehicle) return "The load will be hauled by a sprinter van.";
  return `The load will be hauled by a ${vehicle.name} with limits: Max Weight ${vehicle.maxWeight}lbs, Dimensions (LxWxH): ${vehicle.maxLength}" x ${vehicle.maxWidth}" x ${vehicle.maxHeight}". Take these capabilities into account.`;
};

const DEFAULT_INSTRUCTION = "You are a freight dispatcher assistant. Extract load details accurately. Always provide a 'marketContext' note. The note MUST START with the AI-recommended bid and CPM (e.g., 'AI Recommended Bid: $X,XXX | AI Recommended CPM: $X.XX'), followed by a brief route assessment based on the vehicle specs provided.";
...
export async function POST(req: Request) {
  try {
    ...
    const vehicleContext = getVehicleContext(payload.vehicle);
    ...
    switch (action) {
      case "parseText": {
        ...
        const { marketContext, ...data } = JSON.parse(response.text || "{}");
      ...
      case "getInsight": {
        ...
        const vInfo = vehicle ? `${vehicle.name} (Max Wt: ${vehicle.maxWeight}lbs, Max Dims: ${vehicle.maxLength}"x${vehicle.maxWidth}"x${vehicle.maxHeight}")` : `sprinter van`;
        const response = await ai.models.generateContent({
          model: "gemini-3-flash-preview",
          contents: `Provide a brief market insight for this load given the chosen vehicle: ${vInfo}
          Parameters:
