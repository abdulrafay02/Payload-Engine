...
export default function OutputDisplay({ quote, aiNote, onCommit }: OutputDisplayProps) {
  ...
  return (
    <section className="flex flex-col">
      {/* Primary Output */}
      <div className={`p-3 transition-colors ${hasCriticalWarnings ? 'bg-red-600/20' : 'bg-safety-orange'}`}>
      ...
      {(quote && (quote.warnings.length > 0 || aiNote)) && (
        <div className="grid grid-cols-1 md:grid-cols-2 border-t-2 border-[#888]">
          {quote.warnings.length > 0 && (
            <div className={`bg-red-950/20 p-2 flex flex-col gap-1 ${aiNote ? 'border-r-2 border-[#888]' : ''}`}>
      ...
      {/* Stats Breakdown */}
      <div className="grid grid-cols-2 md:grid-cols-4 border-t-2 border-[#888] bg-industrial-grey/10">
        <div className="flex flex-col p-2 border-r-2 border-[#888]">
